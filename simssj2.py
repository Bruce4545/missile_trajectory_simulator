...
if __name__ == "__main__":
    ...
    sim.numstages = int(input("Number of stages: "))
    sim.FinsNumber = int(input("Number of fins: "))
    for i in range(1,sim.numstages+1):
        sim.fuelmass.append(float(input("Fuel mass: ")))
        drymass = (float(input("Dry mass: ")))
        sim.m0.append(drymass + sim.fuelmass[i])
        sim.fuelfraction.append(sim.fuelmass[i]/sim.m0[i])
        sim.Isp0.append(float(input("Isp: ")))
        sim.thrust0.append(float(input("Thrust (kg f): "))*9.81)
        sim.dMdt.append(float(sim.thrust0[i]/(sim.Isp0[i]*9.81)))
        sim.burntime.append(float(input("Burntime (sec): ")))
        sim.payload = float(input("Payload (kg): "))
        sim.missilediam = float(input("Missile Diameter (m): "))
        sim.nozzlearea = float(input("Nozzle Area (m^2): "))
        sim.LdivD = float(input("Nosecone: Length/Diam (dimensionless): "))
        sim.rvdiam = float(input("Re-entry Diameter (m): "))
        sim.est_range = float(input("Est range (km): "))*1000
        sim.burnout_angle = float(input("Burnout Angle (deg): "))*1
        sim.mach = float(input("Mach: "))*1.


        # ADDED MAY 16
        sim.FinHeight = float(input("Fin Height [m]: "))*1
        sim.SweepAngle = float(input("Sweep Angle [deg]: "))*1
        sim.FinTipChordLength = float(input("Fin Tip Chord Length [m]: "))*1
        sim.FinRootChord = float(input("Fin Root Chord [m]: "))*1
        sim.FinThickness = float(input("Fin Thickness [m]: "))*1
        #
